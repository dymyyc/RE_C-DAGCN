...
from re_cdagcn_model import ReCdagcn
...
def train(args, model, tokenizer, processor, device, n_gpu, results={}):
    ...
    for epoch_num in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(train_iter):
            ...
            input_ids, input_mask, valid_ids, segment_ids, label_ids, e1_mask, e2_mask, dep_type_matrix, pos_type_matrix = batch

            loss = model(input_ids, segment_ids, input_mask, label_ids, e1_mask=e1_mask, e2_mask=e2_mask, valid_ids=valid_ids,
                         dep_adj_matrix=dep_type_matrix, dep_type_matrix=dep_type_matrix, pos_type_matrix=pos_type_matrix)
            if n_gpu > 1:
...
def evaluate(args, model, tokenizer, processor, device, mode="test", output_dir='./'):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        input_ids, input_mask, valid_ids, segment_ids, label_ids, e1_mask, e2_mask, dep_type_matrix, pos_type_matirx = batch

        with torch.no_grad():
            logits = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask, labels=None, e1_mask=e1_mask, e2_mask=e2_mask, valid_ids=valid_ids,
                           dep_adj_matrix=dep_type_matrix, dep_type_matrix=dep_type_matrix, pos_type_matrix=pos_type_matirx)
...
def get_args():
    ...
    parser.add_argument('--dep_type', type=str, default='full_graph', choices=["full_graph", "local_graph", "global_graph", "local_global_graph"])
    parser.add_argument('--num_gcn_layers', type=int, default=2)
    parser.add_argument('--pos_size', type=int, default=1024, help="Dimension of the position type embedding")
    parser.add_argument('--dep_size', type=int, default=1024, help="Dimension of the dependency type embedding")
...
def train_func(args):
    ...
    processor.prepare_dep_dict(args.data_dir)
    processor.prepare_pos_dict(args)
    processor.prepare_labels_dict(args.data_dir)
    label_list = processor.labels_dict.keys()
    pos_type_list = processor.pos_dict.keys()
    dep_type_list = processor.dep_dict.keys()
    dep_num = len(dep_type_list)
    pos_num = len(pos_type_list)
    ...
    config.__dict__["dep_num"] = dep_num
    config.__dict__["pos_num"] = pos_num
    config.__dict__["pos_size"] = args.pos_size
    config.__dict__["dep_size"] = args.dep_size
    model = ReCdagcn(config)
    # model = ReCdagcn.from_pretrained(args.model_path, config=config)
...
def test_func(args):
    ...
    processor = RE_Processor(dep_type=config.dep_type, dep_dict=dict_bin["dep_dict"], pos_dict=dict_bin["pos_dict"], labels_dict=dict_bin["labels_dict"])
    model.to(device)
