...
from model.dagcn import TypeGraphConvolution



class ReCdagcn(BertPreTrainedModel):
    def __init__(self, config):
        super(ReCdagcn, self).__init__(config)
        self.bert = BertModel(config)
        self.dep_type_embedding = nn.Embedding(config.dep_num, config.hidden_size, padding_idx=0)
        self.pos_type_embedding = nn.Embedding(config.pos_num, config.hidden_size)
    ...
    def get_attention(self, val_out, dep_embed, pos_embed, dep_adj):
        batch_size, max_len, feat_dim = val_out.shape
        val_us = val_out.unsqueeze(dim=2)
        val_us = val_us.repeat(1,1,max_len,1)
        val_cat = torch.cat((val_us, dep_embed, pos_embed), -1)
        temp = val_cat.float().transpose(1,2)
        atten_expand = (val_cat.float() * temp)
        ...
        exp_attention_score = torch.mul(exp_attention_score.float(), dep_adj.float())
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, e1_mask=None, e2_mask=None, valid_ids=None, dep_adj_matrix=None, dep_type_matrix=None, pos_type_matrix=None):
        ...
        pos_type_embedding_outputs = self.pos_type_embedding(pos_type_matrix)
        dep_adj_matrix = torch.clamp(dep_adj_matrix, 0, 1)
        for i, gcn_layer_module in enumerate(self.gcn_layer):
            attention_score = self.get_attention(sequence_output, dep_type_embedding_outputs, pos_type_embedding_outputs, dep_adj_matrix)
            sequence_output = gcn_layer_module(sequence_output, attention_score, dep_type_embedding_outputs, pos_type_embedding_outputs)
