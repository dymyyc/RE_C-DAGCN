...
class REDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        def get_pos_matrix(ori_pos_type_matrix):
            pos_type_matrix = np.zeros((self.max_seq_length,self.max_seq_length))
            max_words_num = len(ori_pos_type_matrix)
            for i in range(max_words_num):
                pos_type_matrix[i][:max_words_num] = ori_pos_type_matrix[i]
            return torch.tensor(pos_type_matrix, dtype=torch.long)

        dep_type_matrix = get_dep_matrix(self.data[index]["dep_type_matrix"])
        pos_type_matrix = get_pos_matrix(self.data[index]["pos_type_matrix"])
        return input_ids,input_mask,valid_ids,segment_ids,label_ids,e1_mask_ids,e2_mask_ids, dep_type_matrix, pos_type_matrix

    def __len__(self):
        return len(self.data)

class RE_Processor():
    def __init__(self, direct=True, dep_type="first_order", dep_dict={}, pos_dict={}, labels_dict={}):
        self.direct = direct
    ...
    def get_pos_labels(self, args):
        pos_labels = []
        for pos in range(args.max_seq_length):
            if pos == 0:
                pos_labels.append('{}'.format(pos))
            else:
                pos_labels.append('{}'.format(pos))
                pos_labels.append('-{}'.format(pos))
        return pos_labels

    def get_pos_type_matrix(self, tokens):
        pos_type_matrix = [[str(-j) for j in range(-i,len(tokens)-i)] for i in range(len(tokens))]
        return pos_type_matrix
    ...
    def prepare_dep_dict(self, data_dir):
        dep_type_list = self.get_dep_labels(data_dir)
        dep_dict = {"none": 0}
        for dep_type in dep_type_list:
            dep_dict[dep_type] = len(dep_dict)
        self.dep_dict = dep_dict
        print(dep_dict)

    def prepare_pos_dict(self, args):
        pos_list = self.get_pos_labels(args)
        pos_dict = {}
        for pos in pos_list:
            pos_dict[pos] = len(pos_dict)
        pos_dict['infinity'] = len(pos_dict)
        self.pos_dict = pos_dict
        print(pos_dict)
    ...
    def read_features(self, data_dir, flag):
        ...
        for text_data,dep_info in zip(all_text_data, all_dep_info):
            ...
            pos_type_matrix = self.get_pos_type_matrix(text_data["words"])
            ...
            all_feature_data.append({
                ...
                "pos_type_matrix":pos_type_matrix,
                "label": label,
                "e1":text_data["e1"],
                "e2":text_data["e2"],
            })
    ...
    def convert_examples_to_features(self, examples, tokenizer, max_seq_length):
        ...
        for (ex_index, example) in enumerate(examples):
            ...
            def get_adj_with_value_matrix(dep_adj_matrix, dep_type_matrix, pos_type_matrix):
                ...
                final_pos_type_matrix = np.full((max_words_num, max_words_num),fill_value=2*128-1, dtype=np.int)
                for pi in range(max_words_num):
                    for pj in range(max_words_num):
                        final_pos_type_matrix[pi][pj] = pos_label_map[pos_type_matrix[pi][pj]]
                ...
                return final_dep_adj_matrix, final_dep_type_matrix, final_pos_type_matrix

            dep_adj_matrix, dep_type_matrix, pos_type_matrix = get_adj_with_value_matrix(example["dep_adj_matrix"], example["dep_type_matrix"], example["pos_type_matrix"])
            ...
            if ex_index < 5:
                ...
                logging.info("pos_type_matrix: %s" % " ".join([str(x) for x in pos_type_matrix]))
